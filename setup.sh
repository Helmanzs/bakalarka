...
for db in "${DATABASES_TO_RUN[@]}"; do
  ...
  ansible-playbook setup/setup.yml -e target=$db -e "profile=$PROFILE"
done
  ...
  for db in "${DATABASES_TO_RUN[@]}"; do
    for test in "${TEST_TYPES_TO_RUN[@]}"; do
      ...
      ansible-playbook $ANSIBLE_VERBOSITY "$PLAYBOOK_PATH" \
        -e "database=$db test_type=insert results_dir=$RESULTS_DIR run=$run profile=$PROFILE"

      ansible-playbook $ANSIBLE_VERBOSITY "$PLAYBOOK_PATH" \
        -e "database=$db test_type=$test results_dir=$RESULTS_DIR run=$run profile=$PROFILE"
    done
  done
done